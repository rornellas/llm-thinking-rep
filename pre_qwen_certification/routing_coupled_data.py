...
def _block(domain: str, split: str, doc: int, block: int, rng: random.Random) -> str:
    ...
    if domain == "code":
        return (
            ...
            f"    return state\n"
            f"COMMENT retain pairing, duplicates, and order for {key}\n"
        )
    if domain == "math":
        return (
    ...
    if domain == "portuguese":
        return (
            ...
            f"{d} combina ambas, {e} corrige somente a segunda e {f} confirma o estado combinado. "
    ...
    if domain == "structured":
        return (
            ...
            "MERGE PAIRWISE; RETAIN ORDER; VERIFY COUPLED STATE; CLOSE SET.\n"
        )
    return (
        f"Case {key} combines channel C{a} with channel C{b}. Update {c} changes the left payload, update {d} "
        f"changes the right label, update {e} combines the records, and audit {f} verifies the coupled result.\n"
    )
...
def generate_routing_coupled_ood_documents(*, split: str) -> list[Document]:
    templates = (
        ...
        ("portuguese", "Duas filas recebem os lotes 193, 277, 359 e 467. A confirmação 557 combina as filas sem apagar as correções anteriores. Explique o estado combinado.\n"),
        ...
        ("structured", "<coupled id='C811'><a>A53</a><b>B79</b><merge>857</merge><check>947</check></coupled><rule>preserve-both-then-validate</rule>\n"),
