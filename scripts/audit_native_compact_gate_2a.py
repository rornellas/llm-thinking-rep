#!/usr/bin/env python3
"""Independent-audit entry point with article-level paired semantics."""
...
from scripts import audit_native_compact_gate_2a_impl as runner
...
def paired_rows(
    ...
    """Independently average windows/chunks within seed×article before pairing."""
    ...
    def aggregate(rows: Sequence[Mapping[str, Any]]) -> dict[tuple[int, str], float]:
        ...
        for row in rows:
            buckets[(int(row["seed"]), str(row["document_id"]))].append(float(row["loss"]))
