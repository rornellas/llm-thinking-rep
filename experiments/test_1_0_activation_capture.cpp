...
static bool is_target_name(const std::string & name) {
    return name.find("ffn_norm-7")     != std::string::npos ||
           name.find("ffn_moe_out-7") != std::string::npos;
}

static bool is_small_router_candidate(const ggml_tensor * t, const std::string & name) {
    const bool interesting =
        name.find("expert") != std::string::npos ||
        name.find("router") != std::string::npos ||
        name.find("probs")  != std::string::npos ||
        name.find("ids")    != std::string::npos;
    const bool layer7 = name.find("-7") != std::string::npos;
    return interesting && layer7 && ggml_nelements(t) <= 64 * 2048;
}

static float convert_value(const uint8_t * raw, ggml_type type, size_t index) {
    switch (type) {
        case GGML_TYPE_F32:
            return reinterpret_cast<const float *>(raw)[index];
        case GGML_TYPE_F16:
            return ggml_fp16_to_fp32(reinterpret_cast<const ggml_fp16_t *>(raw)[index]);
        case GGML_TYPE_BF16:
            return ggml_bf16_to_fp32(reinterpret_cast<const ggml_bf16_t *>(raw)[index]);
        case GGML_TYPE_I32:
            return float(reinterpret_cast<const int32_t *>(raw)[index]);
        case GGML_TYPE_I16:
            return float(reinterpret_cast<const int16_t *>(raw)[index]);
        case GGML_TYPE_I8:
            return float(reinterpret_cast<const int8_t *>(raw)[index]);
        default:
            return 0.0f;
    }
}
...
static bool capture_cb(struct ggml_tensor * t, bool ask, void * user_data) {
    ...
    const bool target = is_target_name(name) || is_small_router_candidate(t, name);
    ...
    if (ask) {
        if ((name.find("ffn") != std::string::npos ||
             ...
             name.find("router") != std::string::npos ||
             name.find("probs") != std::string::npos) &&
            state->printed_names.insert(name).second) {
...
int main(int argc, char ** argv) {
    ...
    capture_state state;
    state.out_dir = std::getenv("CAPTURE_DIR") ? std::getenv("CAPTURE_DIR") : "activation_capture";
